...
export const OrderUnverifScreen: React.FC = ({ navigation }: any) => {
    const { result, SearchInput } = useOrder({
        orderStatus: OrderStatusCode.Unverified,
    });
                    ...
                    <>
                        <Heading size="md" mt="6">
                            {data?.orders.result?.length
                                ? data?.orders.result?.length + " "
                                : null}
                            Pesanan Belum terverifikasi
                        </Heading>
                        <Text>
                            {data?.orders.result?.length
                                ? "Kamu punya pesanan yang mungkin harus diverifikasi"
                                : "Kamu ga punya pesanan yang harus diverifikasi"}
                        </Text>
